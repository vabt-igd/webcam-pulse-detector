...
def ocv_imshow(*args, **kwargs):
    return cv2.imshow(*args, **kwargs)


def ocv_destroy_window(*args, **kwargs):
    return cv2.destroyWindow(*args, **kwargs)


def ocv_wait_key(*args, **kwargs):
...
def plot_x_y(data, size=(280, 640), margin=25, name="data", labels=[], skip=[], showmax=[], bg=None, label_ndigits=[],
             showmax_digits=[]):
    for x, y in data:
